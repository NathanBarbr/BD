...
CREATE TABLE Game (
    ...
    Game_type VARCHAR(50) NOT NULL, -- Ex: 'Final', 'Regular Season'
    Season VARCHAR(50),
    ID_Lea INT, -- Le match appartient à une ligue
    ID_Cha INT, -- OU à un championnat
    CONSTRAINT FK_Game_League FOREIGN KEY (ID_Lea)
        REFERENCES League(ID_Lea),
    CONSTRAINT FK_Game_Championship FOREIGN KEY (ID_Cha)
        REFERENCES Championship(ID_Cha)
);



/* [NOUVEAU] Table centrale qui résout les problèmes des requêtes 4 et 7 */
/* Elle identifie QUI a joué (Club/National) et quel SCORE ils ont fait */
...
CREATE TABLE Has_Sponsor_Club (
    ...
    CONSTRAINT FK_HSC_Club FOREIGN KEY (ID_Clu)
        REFERENCES Clubs(ID_Clu),
    CONSTRAINT FK_HSC_Sponsor FOREIGN KEY (ID_Spo)
        REFERENCES Sponsor(ID_Spo)
);
...
CREATE TABLE Has_Sponsor_Team (
    ...
    CONSTRAINT FK_HST_Team FOREIGN KEY (ID_Nat)
        REFERENCES National_team(ID_Nat),
    CONSTRAINT FK_HST_Sponsor FOREIGN KEY (ID_Spo)
        REFERENCES Sponsor(ID_Spo)
);
...
CREATE TABLE Member_of (
    ...
    CONSTRAINT FK_Member_Player FOREIGN KEY (ID_Pla)
        REFERENCES Player(ID_Pla),
    CONSTRAINT FK_Member_Team FOREIGN KEY (ID_Nat)
        REFERENCES National_team(ID_Nat)
);
...
CREATE TABLE Participates_in (
    ...
    CONSTRAINT FK_Participates_Champ FOREIGN KEY (ID_Cha)
        REFERENCES Championship(ID_Cha),
    CONSTRAINT FK_Participates_Team FOREIGN KEY (ID_Nat)
        REFERENCES National_team(ID_Nat)
);
...
CREATE TABLE Participates_in_League (
    ...
    CONSTRAINT FK_PIL_League FOREIGN KEY (ID_Lea)
        REFERENCES League(ID_Lea),
    CONSTRAINT FK_PIL_Club FOREIGN KEY (ID_Clu)
        REFERENCES Clubs(ID_Clu)
);
...
CREATE INDEX IDX_GP_Participant ON Game_Participant(Participant_ID, Participant_Type);
